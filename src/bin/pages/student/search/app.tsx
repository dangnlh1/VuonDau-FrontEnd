...
const AppPage: React.FC<Props> = React.memo(() => {
  ...
  const searchCourseResponse: searchModels.SearchCourseResponse = useMemoriedSelector(
    searchSelectors.searchCourse
  ).response
  ...
  const renderItem = () => {
    if (data && data.length > 0) {
      return data.map((item: CourseItem, index: number) => (
        <CourseItemComponent className="search__course__item__container" data={item} key={index} />
      ))
    }
    return <div className="search__course__not__found">Không tìm thấy kết quả nào!</div>
  }
