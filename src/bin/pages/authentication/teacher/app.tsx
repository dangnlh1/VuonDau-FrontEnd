...
import { genderDropdown, voiceDropdown, levelDropdown, STATUS_CODES } from '@/bin/common/constants'
...
const AppPage: React.FC<Props> = React.memo(() => {
  ...
  const teacherRegisterResponse: authenticationModels.TeacherRegisterResponse = useMemoriedSelector(
    authenticationSelectors.teacherRegister
  ).response
  ...
  useEffect(() => {
    if (teacherRegisterResponse.status) {
      ...
      resetTeacherRegister(authenticationModels.initState.teacherRegister.response)
    }
      ...
      <Content>
        <FlexboxGrid justify="center">
          <FlexboxGrid.Item colspan={12}>
            <Panel header={<h3>Đăng ký giảng dạy trên {process.env.REACT_APP_TITLE}</h3>} bordered>
              <FormProvider {...methods}>
                <FormInput name="username" label="Họ & tên" required />
                <InputDateTime label="Ngày sinh" name="birthDay" isMarginTop required />
                <FormInput name="email" label="Email" isMarginTop required />
                <FormInputNumber label="Số điện thoại" name="phone" isMarginTop required />
                <Dropdown items={genderDropdown} label="Giới tính" name="gender" isMarginTop />
                <Dropdown
                  items={voiceDropdown}
                  label="Giọng nói"
                  name="voice"
                  ...
                  required
                />
                <FormInput label="Nguyên quán" name="domicile" isMarginTop required />
                <FormInput label="Địa chỉ hiện tại" name="currentAddress" isMarginTop required />
                <FormInput
                ...
                <FormInput
                  ...
                  required
                />
                <Dropdown items={levelDropdown} label="Hiện là" name="level" isMarginTop required />
                <ButtonToolbar
                  style={{
                    marginTop: 20,
                  }}
