...
export const useGlobalSelector = () => {
  const apiLoading: boolean = useMemoriedSelector(selectorsLoading.getLoadingIsEnabled)

  const statusError: StatusErrorType = useMemoriedSelector(errorSelectors.statusError)
  ...
  const pageLoading: boolean = useMemoriedSelector(layoutSelectors.getPageLoading)

  const layoutType: ILayoutType = useMemoriedSelector(layoutSelectors.getLayoutType)
