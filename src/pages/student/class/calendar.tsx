...
import {dayOfWeeks, initClassDetail, STATUS_CODES} from '@/common/constants';
...
const CalendarComponent: React.FC<Props> = React.memo(() => {
  ...
  const renderCell = (date: Date) => {
    ...
    const result = _timeToCalendar.find(
      (i: ITimeTableCustom) => i.day === _date,
    );
    if (result?.day) {
      return (
        <div className="calendar__item__container">
          <div className="calendar__item__content">
            <div className="calendar__item__slot__name">
              {result.archetypeTime.archetype.name}
            </div>
