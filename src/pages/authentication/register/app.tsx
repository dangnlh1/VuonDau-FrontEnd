...
const AppPage: React.FC<Props> = React.memo(() => {
  ...
  const studentRegisterResponse: authenticationModels.StudentRegisterResponse =
    useMemoriedSelector(authenticationSelectors.studentRegister).response;
  ...
  useEffect(() => {
    if (studentRegisterResponse.status) {
      ...
      resetStudentRegister(
        authenticationModels.initState.studentRegister.response,
      );
    }
