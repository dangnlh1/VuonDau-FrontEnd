...
const AppPage: React.FC<Props> = React.memo(() => {
  ...
  const createTimeTableResponse: timeTableModels.CreateTimeTableResponse =
    useMemoriedSelector(timeTableSelectors.createTimeTable).response;
  ...
  const getSlotRequest = useCallback(
    (request: timeTableModels.SlotRequest) =>
      dispatch(timeTableActions.slot.request(request)),
    [dispatch],
  );
