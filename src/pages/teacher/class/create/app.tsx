...
import {useCommon, useMemoriedSelector} from '@/hooks';
...
const AppPage: React.FC<Props> = React.memo(() => {
  ...
  const courseSubjectListResponse: courseModels.CourseSubjectListResponse =
    useMemoriedSelector(courseSelectors.courseSubjectList).response;
  ...
  useEffect(() => {
    if (courseSubjectListResponse.status === STATUS_CODES.OK) {
      if (
        courseSubjectListResponse.data &&
        Array.isArray(courseSubjectListResponse.data)
      ) {
